/* ---------- Data ---------- */
let levels = []; // will be fetched from server
...
async function fetchLevels() {
  try {
    const res = await fetch('/api/levels', { headers: jwtToken ? { 'Authorization': `Bearer ${jwtToken}` } : {} });
    if(!res.ok) throw new Error('Failed to fetch levels');
    levels = await res.json();
    renderLevelsView(document.getElementById('list'), levels);
  } catch(err) {
    console.error(err);
    showToast('Failed to load levels', true);
  }
}
...
function renderLevelsView(container, levelsArr) {
    ...
    if(isAdmin){
      ...
      delBtn.addEventListener('click', async e=>{
        ...
        if(confirm(`Delete level #${level.rank} - "${level.title}"?`)){
          try {
            const res = await fetch(`/api/levels/${level.rank}`, {
              method:'DELETE',
              headers:{ 'Authorization': `Bearer ${jwtToken}` }
            });
            if(!res.ok) throw new Error('Failed to delete level');
            await fetchLevels();
            showToast(`Level #${level.rank} deleted`);
          } catch(err) {
            console.error(err);
            showToast('Failed to delete level', true);
          }
        }
      });
    }
  ...
  if(isAdmin){
    ...
    btn.className='px-3 py-1 bg-blue-600 text-white rounded hover:bg-blue-500 text-sm mt-2';
    btn.onclick = async ()=>{
      ...
      try {
        ...
        console.error(err);
        showToast('Failed to create level', true);
      }
    };
...
editForm.addEventListener('submit', async e=>{
  ...
  if(newRank !== oldRank){
    const targetLevel = levels.find(l => l.rank === newRank);
    if(targetLevel) targetLevel.rank = oldRank;
    lvl.rank = newRank;
  }

  try {
    const res = await fetch(`/api/levels/${lvl.rank}`, {
      method:'PUT',
      headers:{
        'Content-Type':'application/json',
        'Authorization': `Bearer ${jwtToken}`
    ...
    showToast(`Level #${lvl.rank} updated`);
  } catch(err){
    console.error(err);
    showToast('Failed to update level', true);
  }
});
